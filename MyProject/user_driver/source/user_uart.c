...
static void handle_read_holding_registers(uint16_t start_address) {
	...
	for (uint16_t i = 0; i < reg_count; i++) {
		tx_buffer[3 + i*2] = 0x00;
		tx_buffer[4 + i*2] = i; // 샘플 데이터
	}
	...
	// 응답 전송
	usart_async_get_io_descriptor(&USART_0, &io);
	rs485_set_tx_mode();  // 송신 모드로 전환

	io_write(io, tx_buffer, 5 + reg_count * 2);
}
...
// // UART 송신 함수 수정
// static void modbus_send_response(uint8_t *buffer, uint16_t length) {
// 	struct io_descriptor *io;
...
// 	// 송신 완료 콜백에서 수신 모드로 전환
// }

// 데이터 송신 함수
// static void send_modbus_response(uint8_t *buffer, uint16_t length) {
//     struct io_descriptor *io;
